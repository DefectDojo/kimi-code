...
describe('AgentPermissionPolicyService chain', () => {
  ...
  it('leaves the dangerous command policy dormant in auto mode on a non-interactive host', async () => {
  ...
  it('still guards a non-interactive host that is not in auto mode', async () => {
    hostArgs = { ...hostArgs, nonInteractive: true };
    mode = 'yolo';
  ...
  it.each(['shutdown -h now', 'rm -r dir'])(
    'does not auto-approve Bash `%s` in auto mode without the opt-in',
    async (command) => {
      mode = 'auto';
